...
class UNet(torch.nn.Module):
    # Spleeter picks activations from config. 2stems ships `params: {}` -> the
    # defaults LeakyReLU(0.2)/ReLU. 4stems ships conv_activation=ELU and
    # deconv_activation=ELU. Same architecture, same weight shapes, so a wrong
    # choice loads cleanly and silently returns garbage -- which is exactly why
    # sherpa-onnx's hardcoded unet.py is 2stems-only.
    def __init__(self, conv_act="LeakyReLU", deconv_act="ReLU"):
        super().__init__()
        self.conv_act = conv_act
        self.deconv_act = deconv_act
    ...
    def _dec(self, x):
        if self.deconv_act == "ELU":
            return torch.nn.functional.elu(x)
        if self.deconv_act == "LeakyReLU":
            return torch.nn.functional.leaky_relu(x, negative_slope=0.2)
        return torch.nn.functional.relu(x)
